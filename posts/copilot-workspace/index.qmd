...
The evidence above suggests there’s still a long way to go before AI-powered tools produce code projects unsupervised that are equivalent to that from a research software engineer. One issue is the generative AI has training database that leans towards common problems, which can makes its solutions to coding problems with specific constraints suboptimal, particularly in fields where there is a relatively small public code base to train models with (e.g. R functions for epidemic analysis are much rarer than generic JavaScript functions for web design). A second is that AI-models are picking up common patterns and synthesising them, and didn’t seem to be able to understand how different parts of a codebase link together, which a developer would be able to use to reduce duplication and keep a consistent design pattern. Put together, we think the applicability of current generative AI models to aid research software engineering in domain-specific R package development, where problems can be complex bespoke solutions are common, is likely to be low. This could improve for use-cases which seek to implement more common tasks, such as building dashboard templates. 
