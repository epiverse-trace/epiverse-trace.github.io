...
We use renv to ensure reproducibility of the training materials and the pipelines we are providing.
But we sometimes get reports from users who struggle to rebuild the environment and run the code.
In this post, we dissect the source of these issues, explain why in reality renv is not at fault, and how this is caused by the inherent complexity of **reproducibility**.
We finish by mentioning a couple of more complete (but more complex!) frameworks that can overcome the issues presented here.
We do not explore these alternative framework in details but we provide links to more information.
...
If you are installing as source code, you may in some case need a compilation toolchain on your computer.
If the toolchain is missing, it can lead to errors such as:
...
We [discussed previously the topic of system dependencies](../system-dependencies/), and dependencies on specific R versions. These special dependencies can also be a source of headaches when using renv.

The heart of the issue is that renv provides a simplified solution to reproducibility: it focuses on R packages and R package versions.
...
But sometimes, it is possible that the `renv.lock` lockfiles requires such an old version of an R package that it was written with a syntax that is no longer supported by recent R versions or modern compilers.
...
Another solution could be to add more complex and more complete reproducibility solutions than renv.
...
Alternatively, you could use other reproducibility toolkit that focus not just on the R package versions, but on the entire software stack.
These solutions can be more complex to setup and use, and we won't detail them in this blog post but you can find more information in:
