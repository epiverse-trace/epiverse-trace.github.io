...
With this distinction in mind, we can now focus on why you should favor
...
By consistently employing `apply()` for iteration and reserving `for` loops for
...
patterns. This can be demonstrated using R's most popular static analysis tool:
the lintr package.
...
lengths_vapply <- "vapply(l, length, numeric(1))"

lintr::lint(text = lengths_for, linters = lintr::lengths_linter()) # pass

lintr::lint(text = lengths_vapply, linters = lintr::lengths_linter()) # lint
```
