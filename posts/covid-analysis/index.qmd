---
title: "How well are we prepared to analyse an influenza pandemic? A brief perspective on analysis conducted for UK government advisory groups during COVID-19"
author:
  - name: "Adam Kucharski"
...
This was a paper for SPI-M-O: **Feasibility of controlling 2019-nCoV outbreaks by isolation of cases and contacts**, later published as [Hellewell et al, Lancet Global Health](https://www.thelancet.com/article/S2214-109X(20)30074-7/fulltext). It used a branching process model to explore how transmission (e.g. $R_0$ and % presymptomatic transmission) and control parameters (e.g. % contacts traced) could influence the risk of a large outbreak.
...
This analysis estimating the infection and case fatality ratio by age for COVID-19 using age-adjusted data from the outbreak on the Diamond Princess cruise ship. Later published as [Russell et al, Eurosurveillance, 2020](https://www.eurosurveillance.org/content/10.2807/1560-7917.ES.2020.25.12.2000256).

The methods used for this analysis are now implemented as the Epiverse-TRACE [\{cfr\} package](https://github.com/epiverse-trace/cfr/). There is also a ['how to' example for age-specific CFR estimation](https://epiverse-trace.github.io/howto/analyses/estimate_severity/cfr-stratified-severity.html).

**Suggested or ongoing development:** There are several [issues](https://github.com/epiverse-trace/cfr/issues) that would help strengthen \{cfr\}, especially for edge cases.
...
**Suggested or ongoing development:** For a future epidemic, it may be more useful to merge these concepts into two types of tool, building on succesful outputs for COVID: 1) a ['ready reckoners' package](https://royalsocietypublishing.org/doi/10.1098/rstb.2020.0276) that shows very intuitively how contact changes influence overall R, and 2) a more comprehensive model of isolation and quarantine, like the prospective [\{epinetwork\}](https://github.com/epiverse-trace/epinetwork/) package.
...
Although the underlying model isn't in a package, the analysis is featured in the [Epiverse-TRACE training](https://epiverse-trace.github.io/tutorials-middle/superspreading-estimate.html#controlling-superspreading-with-contact-tracing).

**Suggested or ongoing development:** The core insight is also a relatively simple equation, i.e. that backward tracing would be expected to identify $R(1+1/k)$ cases, where $k$ is the dispersion parameter, so should add this to relevant training and/or vignettes.
